...
#전처리과정 melt 대신 이렇게 사용가능한 것을 보여준 예시####
...
dotplot(Var1 ~ Freq, data = dft,
        groups = Var2, type = "o",auto.key = list(space = "right", points = T, lines = T))
...
convert <- transform(quakes, depth2 = factor(depth2))#transform은 전처리 과정에서 유용
...
numgroup <- equal.count(1:150, number = 4, overlap = 0)#equal.count는 4개로 나누어 범주화한다.
numgroup
...
#5단계: 산점도 그래프에 cyl변수의 요인으로 포인트 크기 적용 & carb변수의 요인으로 포인트 색적용
qplot(wt, mpg, data = mtcars, size = factor(cyl),color = factor(carb), geom = "point")

#6단계: 산점도 그래프에 qsec변수의 요인으로 포인트 크기 적용 & cyle변수의 요인으로 포인트모양 적용
...
#실습(geom_line()과 geom._point()함수를 적용하여 레이어 추가)


#1단계: geom._line()레이어 추가
p <- ggplot(mtcars, aes(mpg, wt, color = factor(cyl)))
p + geom_line()

#2단계: geom._point레이어 추가
p <- ggplot(mtcars, aes(mpg, wt, color = factor(cyl)))
p + geom_point()
